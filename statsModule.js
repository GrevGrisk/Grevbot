...
function extractSteam64(serverPlayer, userLookup, altPlayer) {
    return (
        altPlayer?.steam64 ||
        serverPlayer?.gamedata?.steam64 ||
        serverPlayer?.data?.gamedata?.steam64 ||
        serverPlayer?.player?.gamedata?.steam64 ||
        serverPlayer?.steam64 ||
        serverPlayer?.data?.steam64 ||
        userLookup?.steam64 ||
        userLookup?.data?.steam64 ||
        userLookup?.profile?.steam64 ||
        userLookup?.data?.profile?.steam64 ||
        null
    );
}
...
async function handleProfile(interaction) {
    ...
    try {
        ...
        if (!steamCreated) {
            ...
            const steam64 = extractSteam64(serverPlayer, userLookup, altPlayer);

            steamCreated =
                await getSteamAccountCreated(steam64) ||
